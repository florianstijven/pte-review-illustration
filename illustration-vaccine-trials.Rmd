...
discrepancy between the treatment effect on $\Tilde{S}$ and $T$. The treatment
effect on $\Tilde{S}$ is basically a prediction for the treatment effect on
$T$. This is shown in Lemma 4.2 in the review paper. In Table
\@ref(tab:application-trial-pte-results), the "true" and predicted treatment
effects are summarized in both application trials. The predicted treatment
...
expect the true and predicted treatment effects to match. Indeed, $PTE_{WT}$ is
not close to $1$ in the evaluation trial. So, by Lemma 4.1 in the review paper,
the predicted treatment effect would also have been far away from the true
treatment effect **in the evaluation trial**. So, we can certainly not expect
better predictions in application trials. 

In the application trial, we saw that $PTE_{WT}^X \approx 1$. Hence, the
predicted treatment effect in the application trial matches the true treatment
effect,
...
expect accurate predictions. These conditions are only satisfied in application
trial 1. Indeed, the corresponding predicted treatment effect shown in Table
...
prediction. The latter is confirmed in Table \@ref(tab:application-trial-pte-results).
